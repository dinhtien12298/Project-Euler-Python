# Tích lớn nhất theo các hướng của 4 số
...
# Thuật toán thêm dãy số trên vào 1 list
...
# Thuật toán tìm tích của hướng lơn nhất
list_value = []
...
for index, item in enumerate(number_final):
    ...
    for i in range(0, 80, 20):
        value1 *= int(number_final[index-i])
    for i in range(0, 4):
        value2 *= int(number_final[index-i])
    for i in range(0, 76, 19):
        value3 *= int(number_final[index-i])
    for i in range(0, 84, 21):
        value4 *= int(number_final[index-i])
    list_value.append(value1)
    list_value.append(value2)
    list_value.append(value3)
